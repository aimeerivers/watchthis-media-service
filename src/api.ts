...
import { normalizeUrl, detectPlatform, validateUrl } from "./utils/urlProcessor.js";
...
export function mountApi(mountRoute: string, app: Express): void {
  ...
  // Get media by ID
  app.get(mountRoute + "/media/:id", asyncHandler(async (req: Request, res: Response) => {
    const { id } = req.params;
  ...
  app.patch(mountRoute + "/media/:id", asyncHandler(async (req: Request, res: Response) => {
    const { id } = req.params;
  ...
  // Delete media item
  app.delete(mountRoute + "/media/:id", asyncHandler(async (req: Request, res: Response) => {
    const { id } = req.params;
  ...
  app.get(mountRoute + "/media", asyncHandler(async (req: Request, res: Response) => {
    const page = parseInt(req.query.page as string) || 1;
